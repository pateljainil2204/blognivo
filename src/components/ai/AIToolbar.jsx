...
export default function AIToolbar({ content, onApply, onTagsSuggested, onTitlesSuggested }) {
  ...
  return (
    <div className="border border-indigo-500/30 rounded-2xl p-6 bg-[#0f172a] shadow-2xl shadow-indigo-500/10 space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-700 ring-1 ring-white/5">
      ...
      <div className="flex flex-wrap gap-2">
        {AI_ACTIONS.map(({ key, label, icon: Icon }) => (
          <button
            ...
            className="group flex items-center gap-2 px-4 py-2 text-xs font-bold rounded-xl border border-white/10 bg-white/10 hover:bg-indigo-500/20 hover:border-indigo-500/40 hover:text-white disabled:opacity-20 disabled:cursor-not-allowed transition-all duration-300 shadow-md"
          >
      ...
      {result && (
        <div className="bg-[#1e1b4b] border border-indigo-500/30 rounded-2xl shadow-3xl p-6 animate-in zoom-in-95 duration-500">
          ...
          {result.action === 'title' ? (
              ...
              <div className="grid gap-2">
                {result.titles.map((title, i) => (
                  <button
                    ...
                    className="w-full text-left p-3 text-sm rounded-xl border border-white/10 bg-white/10 hover:border-indigo-500/30 hover:bg-indigo-500/20 text-gray-300 hover:text-white transition-all duration-300"
                  >
                    {title}
                  </button>
                ))}
            ...
            <div className="space-y-6">
              <div className="text-sm text-gray-100 whitespace-pre-wrap leading-relaxed max-h-80 overflow-y-auto pr-4 custom-scrollbar font-medium bg-[#020617] p-6 rounded-2xl border border-white/10 selection:bg-indigo-500/50">
                {result.text}
              </div>
