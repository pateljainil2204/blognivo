...
export default function AIModeration({ decision, reason, score }) {
  ...
  return (
    <div className={`rounded-2xl border p-4 mb-6 shadow-sm overflow-hidden relative ${
      isApproved ? 'bg-green-50/30 border-green-100' : 'bg-red-50/30 border-red-100'
    }`}>
      {/* Background Icon */}
      ...
      <div className="relative flex items-start gap-4">
        <div className={`mt-1 p-2 rounded-xl border ${
          isApproved ? 'bg-green-100/50 border-green-200 text-green-600' : 'bg-red-100/50 border-red-200 text-red-600'
        }`}>
        ...
        <div className="flex-1">
          ...
          <p className={`text-sm leading-relaxed font-medium ${
            isApproved ? 'text-green-700/80' : 'text-red-700/80'
          }`}>
            {reason}
          </p>
        </div>
